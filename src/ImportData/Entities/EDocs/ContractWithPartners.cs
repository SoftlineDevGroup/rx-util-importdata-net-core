...
{
    public class ContractWithPartners : Entity
    {
        public int PropertiesCount = 23;
        /// <summary>
        ...
        public override IEnumerable<Structures.ExceptionsStruct> SaveToRX(Logger logger, bool supplementEntity, string ignoreDuplicates, int shift = 0)
        {
            ...
            // 20 Примечание
            var note = this.Parameters[shift + 19];
            logger.Debug($"Примечание {note}");
            logger.Debug($"шифт - {shift}");
            logger.Debug($"количество - {this.Parameters.Length}");
            logger.Debug($" ЖР - {this.Parameters[shift + 20].Trim()}");

            // 21 Журнал регистрации
            var documentRegisterIdStr = this.Parameters[shift + 20].Trim();
            logger.Debug($"Журнал регистрации {documentRegisterIdStr}");
            ...
            // 22 Состояние регистрации
            var regState = this.Parameters[shift + 21].Trim();
            logger.Debug($"Состояние регистрации {regState}");

            // 23 Внешний ИД
            var extId = this.Parameters[shift + 22].Trim();
            logger.Debug($"Внешний ИД {extId}");
