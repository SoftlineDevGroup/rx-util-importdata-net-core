    ...
    public class BusinessLogic
    {
        ...
        public static IEnumerable<T> GetEntitiesWithFilter<T>(Expression<Func<T, bool>> expression, List<Structures.ExceptionsStruct> exceptionList, Logger logger, bool isExpand = false) where T : class
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error($"GetEntitiesWithFilter > Ошибка при получении сущностей {PrintInfo(typeof(T))}");
            }
        ...
        public static IEnumerable<T> GetEntities<T>(List<Structures.ExceptionsStruct> exceptionList, Logger logger) where T : class
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error($"GetEntities > Ошибка при получении сущностей {PrintInfo(typeof(T))}");
            }
            return null;
        }
        ...
        public static T CreateEntity<T>(T entity, List<Structures.ExceptionsStruct> exceptionList, Logger logger) where T : class
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error($"CreateEntity > Ошибка при создании сущности {PrintInfo(typeof(T))}, Message {ex.Message}, InnerException {ex.InnerException}");
            }
        ...
        public static T UpdateEntity<T>(T entity, List<Structures.ExceptionsStruct> exceptionList, Logger logger) where T : class
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Error($"UpdateEntity > Ошибка при обновлении сущности {PrintInfo(typeof(T))}, Message {ex.Message}, InnerException {ex.InnerException}");
            }
            return null;
        }
        #endregion
        ...
        public static IEnumerable<Structures.ExceptionsStruct> ImportBody(IElectronicDocuments edoc, string pathToBody, Logger logger, bool update_body = false)
        {
            ...
            try
            {
                ...
                var associatedApplication = BusinessLogic.GetEntityWithFilter<IAssociatedApplications>(a => a.Extension.ToLower() == extention.ToLower(), exceptionList, logger);
                logger.Debug($"Приложение-обработчик {associatedApplication?.Id}");

                if (associatedApplication != null)
                {
                    //var lastVersion = edoc.LastVersion();
                    //if (lastVersion == null || !update_body || lastVersion.AssociatedApplication.Extension.ToLower() != extention.ToLower())
                    var lastVersion = edoc.CreateVersion(edoc.Name, associatedApplication);
                    ...
                    logger.Debug($"Тело загружено {isFillBody}");
                }
                else
                {
        ...
        /// <param name="lifeCycleStateName">Наименование ЖЦ документа.</param>
        /// <returns>ЖЦ.</returns>
        public static string GetPropertyAccountDetails(string key)
        {
            Dictionary<string, string> AccountDetails = new Dictionary<string, string>
        {
            {"По договорам", "ByContractsline"},
            {"По заказам", "ByOrdersline"},
            {"Аванс по договорам, долг по накладным", "AdvanceContractsline"},
            {"Аванс по заказам, долг по накладным", "AdvanceOrdersline"},
            {"По расчетным документам", "BySettlementDocsline"},
            {"", null}
        };
